...
def projected_completion(
    *, remaining_bytes: int, snapshot: ThroughputSnapshot
) -> timedelta | None:
    if remaining_bytes <= 0:
        return timedelta(0)
    if snapshot.files_completed == 0 or snapshot.bytes_per_second <= 0:
        return None
...
def failure_rate(*, registry: Registry, batch_id: str | None = None) -> float:
    ...
    failed = counts_sum(counts, "FAILED", "ABANDONED", "ARCHIVE_FAILED")
...
def counts_sum(counts: Mapping[object, int], *names: str) -> int:
    return sum(value for key, value in counts.items() if getattr(key, "name", None) in names)
